...
import { Phone, Brain, Calendar, Bell, MessageSquare, RefreshCw, Check } from "lucide-react";
...
export default function WorkflowDiagram() {
  ...
  return (
    <div className="relative w-full max-w-md mx-auto" data-testid="workflow-diagram">
      ...
      <div className="relative bg-slate-900/40 backdrop-blur-xl border border-white/10 rounded-3xl p-6 sm:p-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-xs uppercase tracking-widest text-slate-400 font-medium">Live Workflow</span>
          </div>
          <span className="text-xs text-slate-500">Call → Action</span>
        </div>
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          >
            <div className="w-10 h-10 rounded-lg bg-slate-800 flex items-center justify-center relative">
          ...
          <div className="relative h-8 w-px bg-gradient-to-b from-cyan-400/40 to-blue-500/40 overflow-hidden">
            {!reduced && (
              <motion.div
                className="absolute left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-cyan-400 shadow-[0_0_8px_rgba(34,211,238,0.8)]"
                animate={{ top: ["-10%", "110%"] }}
                transition={loop as any}
              />
            )}
          </div>
          ...
          >
            <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center shadow-lg shadow-cyan-500/30">
              <Brain className="h-5 w-5 text-white" />
            </div>
            <div className="flex-1">
              <div className="text-sm font-semibold text-white">DSX Edge AI Agent</div>
              <div className="text-xs text-cyan-300">Qualifying · Following your workflow</div>
            </div>
          </motion.div>

          <div className="relative h-8 w-full flex items-start justify-center">
            <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none" viewBox="0 0 200 32">
              <path d="M100 0 L100 12 L20 28" stroke="rgba(34,211,238,0.3)" strokeWidth="1" fill="none" />
              <path d="M100 0 L100 12 L73 28" stroke="rgba(34,211,238,0.3)" strokeWidth="1" fill="none" />
              <path d="M100 0 L100 12 L127 28" stroke="rgba(34,211,238,0.3)" strokeWidth="1" fill="none" />
              <path d="M100 0 L100 12 L180 28" stroke="rgba(34,211,238,0.3)" strokeWidth="1" fill="none" />
            </svg>
          </div>
