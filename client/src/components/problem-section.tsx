      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-start mb-14">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            className="bg-white rounded-2xl p-8 border border-slate-200/70 shadow-sm"
          >
            <p className="text-[11px] uppercase tracking-[0.18em] text-slate-400 font-semibold mb-3">
              The buyer's reality
            </p>
            <p className="text-slate-700 text-lg leading-relaxed italic mb-5">
        ...
        <div className="grid sm:grid-cols-2 gap-5">
          {frictions.map((item, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="group relative bg-white border border-slate-200/80 rounded-2xl p-7 hover:border-blue-300 hover:shadow-lg hover:shadow-blue-100/50 hover:-translate-y-0.5 transition-all duration-300"
              data-testid={`problem-card-${i}`}
            >
              <div className="flex items-start gap-5">
                <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-blue-50 to-blue-100/60 border border-blue-100 flex items-center justify-center flex-shrink-0 group-hover:from-blue-100 group-hover:to-blue-200/60 transition-colors">
                  <item.icon className="h-5 w-5 text-blue-700" />
                </div>
                <div className="flex-1">
                  <div className="text-[15px] font-semibold text-slate-900 mb-1.5 leading-snug">
                    {item.label}
                  </div>
